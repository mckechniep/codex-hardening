...
def _nft_chain_exists(family: str, table: str, chain: str, *, use_sudo: bool) -> bool:
    result = subprocess.run(
        _sudo_command(["nft", "list", "chain", family, table, chain], use_sudo),
        check=False,
        capture_output=True,
        text=True,
    )
...
def select_exec_rules_context(config: dict, token: str, *, use_sudo: bool) -> dict[str, str]:
    if _nft_chain_exists("ip", "filter", "FORWARD", use_sudo=use_sudo) and _nft_chain_exists(
        "ip", "nat", "POSTROUTING", use_sudo=use_sudo
    ):
        return {
            ...
            "filter_exec_chain": f"codex_exec_fwd_{token}",
            "nat_family": "ip",
            "nat_table": "nat",
            "nat_chain": "POSTROUTING",
            "nat_exec_chain": f"codex_exec_nat_{token}",
            "forward_jump_comment": f"codex-net:{token}:forward-jump",
            "return_accept_comment": f"codex-net:{token}:return-accept",
            "nat_jump_comment": f"codex-net:{token}:nat-jump",
        }
    return {
        "mode": "standalone_table",
...
def render_exec_rules(
    ...
    if mode == "global_host_chains":
        lines = [
            ...
            f"add chain {rules_context['filter_family']} {rules_context['filter_table']} {rules_context['filter_exec_chain']}",
            (
                f"add rule {rules_context['filter_family']} {rules_context['filter_table']} {rules_context['filter_chain']} "
                f'iifname "{host_veth}" jump {rules_context["filter_exec_chain"]} '
                f'comment "{rules_context["forward_jump_comment"]}"'
            ),
            (
                f"add rule {rules_context['filter_family']} {rules_context['filter_table']} {rules_context['filter_chain']} "
                f'oifname "{host_veth}" ct state established,related accept '
                f'comment "{rules_context["return_accept_comment"]}"'
            ),
        ]
        if profile["allow_localhost"]:
            ...
            if not tcp_ports and not udp_ports:
                lines.append(
                    f'add rule {rules_context["filter_family"]} {rules_context["filter_table"]} {rules_context["filter_exec_chain"]} '
                    f'ip daddr {addresses["host_ip"]} accept'
                )
        ...
        lines.extend(
            [
                ...
                (
                    f"add rule {rules_context['nat_family']} {rules_context['nat_table']} {rules_context['nat_chain']} "
                    f'ip saddr {addresses["subnet"]} oifname != "{host_veth}" jump {rules_context["nat_exec_chain"]} '
                    f'comment "{rules_context["nat_jump_comment"]}"'
                ),
                (
                    f"add rule {rules_context['nat_family']} {rules_context['nat_table']} "
                    f'{rules_context["nat_exec_chain"]} masquerade'
                ),
                "",
            ]
        )
...
def _run_nft_text(rules: str, *, use_sudo: bool) -> subprocess.CompletedProcess[str]:
    try:
        result = subprocess.run(
            _sudo_command(["nft", "-f", "-"], use_sudo),
            check=False,
            capture_output=True,
            text=True,
            input=rules,
        ...
        raise BackendError("nft invocation failed because `nft` is not installed.") from exc

    if result.returncode != 0:
        detail = (result.stderr or result.stdout).strip() or f"exit code {result.returncode}"
        raise BackendError(f"nft invocation failed: {detail}")
    return result


def _delete_nft_rules_by_comment(family: str, table: str, chain: str, comment: str, *, use_sudo: bool) -> None:
    result = subprocess.run(
        _sudo_command(["nft", "-a", "list", "chain", family, table, chain], use_sudo),
        check=False,
        capture_output=True,
        text=True,
    )
    ...
    for line in result.stdout.splitlines():
        if marker not in line or "# handle " not in line:
            continue
        handles.append(line.rsplit("# handle ", 1)[1].strip())

    for handle in handles:
        _run_best_effort(["nft", "delete", "rule", family, table, chain, "handle", handle], use_sudo=use_sudo)


def cleanup_exec_rules(config: dict, token: str, rules_context: dict[str, str], *, use_sudo: bool) -> None:
    if rules_context["mode"] == "global_host_chains":
        _delete_nft_rules_by_comment(
            rules_context["filter_family"],
            rules_context["filter_table"],
            rules_context["filter_chain"],
            rules_context["forward_jump_comment"],
            use_sudo=use_sudo,
        )
        _delete_nft_rules_by_comment(
            rules_context["filter_family"],
            rules_context["filter_table"],
            rules_context["filter_chain"],
            rules_context["return_accept_comment"],
            use_sudo=use_sudo,
        )
        _delete_nft_rules_by_comment(
            rules_context["nat_family"],
            rules_context["nat_table"],
            rules_context["nat_chain"],
            rules_context["nat_jump_comment"],
            use_sudo=use_sudo,
        )
        _run_best_effort(
            ["nft", "delete", "chain", rules_context["nat_family"], rules_context["nat_table"], rules_context["nat_exec_chain"]],
            use_sudo=use_sudo,
        )
        _run_best_effort(
            [
                ...
                rules_context["filter_family"],
                rules_context["filter_table"],
                rules_context["filter_exec_chain"],
            ],
            use_sudo=use_sudo,
        )
        return
...
def run_netns_exec(profile_name: str, wrapped_command: list[str], config: dict, *, use_sudo: bool | None = None) -> int:
    ...
    rules_context = select_exec_rules_context(config, token, use_sudo=use_sudo)
    ...
    if rules_context["mode"] == "global_host_chains":
        record["filter_exec_chain"] = rules_context["filter_exec_chain"]
        record["nat_exec_chain"] = rules_context["nat_exec_chain"]
    ...
    try:
        ...
        _run_nft_text(nft_path.read_text(), use_sudo=use_sudo)
        nft_applied = True
        ...
        if nft_applied:
            cleanup_exec_rules(config, token, rules_context, use_sudo=use_sudo)
